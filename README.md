...
Accordingly, it is recommended to increase security using either the __nonce__ `YqcrfXBSOk0SvJQKhJX+wRW/ZSHz5OVoMrnX6CWEwHc=` or the *integrity* attribute, trusting via [CSP](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy) only scripts that comes from our own domain.

```html
<meta http-equiv="Content-Security-Policy" content="script-src 'self' 'unsafe-eval'">
<script defer src="/js/uce-template.js"
        integrity="sha256-YqcrfXBSOk0SvJQKhJX+wRW/ZSHz5OVoMrnX6CWEwHc="
